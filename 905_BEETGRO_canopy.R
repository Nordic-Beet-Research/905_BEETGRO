...
dat_in_trial_i <- dat_in_trial_i %>%
  mutate(
    qfc = dat_in_param$a2 * (dat_in_param$a1/5)^(1/b),
    qpwp = dat_in_param$a2 * (dat_in_param$a1/1500)^(1/b),
...
# Temperature
  ...
  mutate(
    ...
    bbch = replace(bbch, dat_in_trial_i$EmDOY <= dat_in_trial_i$SowDOY & Cd_sow >= dat_in_param$Tzero, 01)
  ) %>%
...
# Canopy
  mutate(
    f = dat_in_param$fZero,
    WStress = 1,
    WStress = replace(WStress, dat_in_trial_i$qrel < dat_in_trial_i$WstressB & xtemp > dat_in_trial_i$WstressC, 1),
